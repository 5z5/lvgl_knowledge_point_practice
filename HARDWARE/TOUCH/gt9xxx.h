...
#define GT9XXX_RST_GPIO_CLK_ENABLE()    do{ RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOF, ENABLE ); }while(0)   /* PF口时钟使能 */

#define GT9XXX_INT_GPIO_PORT            GPIOF
#define GT9XXX_INT_GPIO_PIN             GPIO_Pin_10
#define GT9XXX_INT_GPIO_CLK_ENABLE()    do{ RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOF, ENABLE ); }while(0)   /* PF口时钟使能 */
...
#define GT9XXX_RST(x)     do{ x ? \
                              GPIO_SetBits(GT9XXX_RST_GPIO_PORT, GT9XXX_RST_GPIO_PIN) : \
                              GPIO_ResetBits(GT9XXX_RST_GPIO_PORT, GT9XXX_RST_GPIO_PIN); \
                          }while(0)       /* 复位引脚 */

#define GT9XXX_INT        GPIO_ReadInputDataBit(GT9XXX_INT_GPIO_PORT, GT9XXX_INT_GPIO_PIN)     /* 读取做的引脚 */
