...
#define FT5206_RST_GPIO_CLK_ENABLE()    do{ RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOF, ENABLE ); }while(0)   /* PF口时钟使能 */

#define FT5206_INT_GPIO_PORT            GPIOF
#define FT5206_INT_GPIO_PIN             GPIO_Pin_10
#define FT5206_INT_GPIO_CLK_ENABLE()    do{ RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOF, ENABLE ); }while(0)   /* PF口时钟使能 */
...
#define FT5206_RST(x)     do{ x ? \
                              GPIO_SetBits(FT5206_RST_GPIO_PORT, FT5206_RST_GPIO_PIN) : \
                              GPIO_ResetBits(FT5206_RST_GPIO_PORT, FT5206_RST_GPIO_PIN); \
                          }while(0)       /* 复位引脚 */

#define FT5206_INT        GPIO_ReadInputDataBit(FT5206_INT_GPIO_PORT, FT5206_INT_GPIO_PIN)  /* 读取做的引脚 */
