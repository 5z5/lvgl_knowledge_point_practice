...
static void tp_adjust_info_show(uint16_t xy[5][2], double px, double py)
{
    ...
    for (i = 0; i < 5; i++)   /* 显示5个物理坐标值 */
    {
        ...
        POINT_COLOR = RED;
        LCD_ShowString(40, 160 + (i * 20), lcddev.width, lcddev.height, 16, (unsigned char *)sbuf);
        sprintf(sbuf, "y%d:%d", i + 1, xy[i][1]);
        POINT_COLOR = RED;
        LCD_ShowString(40 + 80, 160 + (i * 20), lcddev.width, lcddev.height, 16, (unsigned char *)sbuf);
    }
        ...
        if ((tp_dev.sta & 0xc000) == TP_CATH_PRES)   /* 按键按下了一次(此时按键松开了.) */
        {
            ...
            switch (cnt)
            {
                ...
                case 5:     /* 全部5个点已经得到 */
                    ...
                    LCD_Clear(WHITE);   /* 清屏 */
                    POINT_COLOR = RED;
                    LCD_ShowString(35, 110, lcddev.width, lcddev.height, 16, (unsigned char *)"Touch Screen Adjust OK!"); /* 校正完成 */
                    delay_ms(1000);
                    tp_save_adjust_data();
...
uint8_t tp_init(void)
{
    ...
    else
    {
        ...
        gpio_init_struct.GPIO_Pin = T_PEN_GPIO_PIN;
        gpio_init_struct.GPIO_Mode = GPIO_Mode_IPU;                 /* 输入 */
        gpio_init_struct.GPIO_Speed = GPIO_Speed_50MHz;           /* 高速 */
        GPIO_Init(T_PEN_GPIO_PORT, &gpio_init_struct);       /* 初始化T_PEN引脚 */

        gpio_init_struct.GPIO_Pin = T_MISO_GPIO_PIN;
        GPIO_Init(T_MISO_GPIO_PORT, &gpio_init_struct);      /* 初始化T_MISO引脚 */

        gpio_init_struct.GPIO_Pin = T_MOSI_GPIO_PIN;
        gpio_init_struct.GPIO_Mode = GPIO_Mode_Out_PP;             /* 推挽输出 */
        gpio_init_struct.GPIO_Speed = GPIO_Speed_50MHz;           /* 高速 */
        GPIO_Init(T_MOSI_GPIO_PORT, &gpio_init_struct);      /* 初始化T_MOSI引脚 */

        gpio_init_struct.GPIO_Pin = T_CLK_GPIO_PIN;
        GPIO_Init(T_CLK_GPIO_PORT, &gpio_init_struct);       /* 初始化T_CLK引脚 */

        gpio_init_struct.GPIO_Pin = T_CS_GPIO_PIN;
        GPIO_Init(T_CS_GPIO_PORT, &gpio_init_struct);        /* 初始化T_CS引脚 */
